...
other transport `laya-mcp mcp` exposes over stdio.
    ...
    python scripts/verify_laya_integration.py --route       # MLX sidecar's CashCard routing
...
#: `POST /route` queues, mirroring CASHCARD_QUEUES in scripts/laya_mlx_sidecar.py.
ROUTE_QUEUES = {"topup", "pass", "scan", "refund", "plan", "sales", "human_review"}
ROUTE_KEYS = {
    "queue",
    "predicted_queue",
    "confidence",
...
def verify_http(v: Verifier) -> None:
    ...
    pos, ms_pos = v.call(
        "/ask",
        {"state": {"body": "The app crashes every time I upload a CSV."}, "questions": BUG_Q},
    )
    p_pos = pos["answers"]["is_defect"]["noul"]
    v.check(
        "2. noul positive (defect report)",
        p_pos > 0.5 and pos["answers"]["is_defect"].get("band") == "yes",
        f"P(true)={p_pos:.4f} band={pos['answers']['is_defect'].get('band')}",
    )

    neg, ms_neg = v.call(
        "/ask", {"state": {"body": "What payment methods do you accept?"}, "questions": BUG_Q}
    )
    p_neg = neg["answers"]["is_defect"]["noul"]
    v.check(
        "3. noul negative (pricing question)",
        p_neg < 0.5 and neg["answers"]["is_defect"].get("band") == "no",
    ...
    v.check(
        "4. neutral A/B labels answer both directions correctly",
        ab_p == "A" and ab_n == "B",
    ...
    v.info(
        "same positive review under true/false labels",
    ...
    bare, _ = v.call(
        "/ask",
        {
            "state": {"body": "The app crashes every time I upload a CSV."},
            "questions": {
                "is_defect": {
                    "type": "noul",
    ...
    v.check(
        "4b. noul with no option text is still well-formed",
    ...
    v.check(
        "5. choice -> billing",
        a["choice"] == "billing" and a["probabilities"]["billing"] > 0.5,
    ...
    sc, _ = v.call(
        "/ask",
        {
            "state": {"body": "Production is down for all customers and there is no workaround."},
            "questions": PRIO_Q,
        },
    )
    ...
    v.check(
        "6. score places a Sev-1 at the top level",
    ...
    v.check(
        "7. POST /plan (no forward pass)",
        pl.get("ok") and pl.get("fits") and ms_pl < 50,
        f"fits={pl.get('fits')} {ms_pl:.0f} ms (vs {ms_ch:.0f} ms for /ask)",
    )

    plbig, _ = v.call(
        "/plan", {"state": {"body": "lorem ipsum dolor sit amet " * 400}, "questions": RUBRIC_Q}
    )
    q = plbig["questions"][0]
    v.check(
        "8. oversized state is flagged before you ask",
    ...
    def one(i: int) -> float:
        b, _ = v.call(
            "/ask",
            {"state": {"body": f"Ticket {i}: the app crashes on upload."}, "questions": BUG_Q},
        )
    ...
    try:
        ...
        v.check(
            "10. 4 concurrent /ask calls all answered",
            all(x > 0.5 for x in vals),
            f"nouls={[round(x, 3) for x in vals]}",
        )
    except Exception as e:
        v.check(
            "10. 4 concurrent /ask calls all answered", False, f"raised {type(e).__name__}: {e}"
        )
    ...
    if ms_pos > 500:
        v.info(
            "first inference on a cold process",
    ...
    v.check(
        "11. steady-state latency is sub-200ms",
        p50 < 200,
...
def verify_route(v: Verifier) -> None:
    """`POST /route`: CashCard support routing, served only by the MLX sidecar.

    The messages are ones the English checkpoint routes with high confidence in
    the 32-message CashCard test set, so a failure here is a regression, not noise.
    """
    try:
        gate, _ = v.call(
            "/route",
            {"message": "Bhai gate pe scan nahi ho raha, guard andar nahi jaane de raha!"},
        )
    except urllib.error.HTTPError as e:
        v.check("R1. POST /route shape", False, f"HTTP {e.code}: {e.read().decode()[:200]}")
        if e.code == 404:
            print("\n/route is served by scripts/laya_mlx_sidecar.py, not `laya-mcp serve`.")
        return
    missing = ROUTE_KEYS - set(gate)
    v.check(
        "R1. POST /route shape",
        gate.get("ok") is True and not missing and gate.get("queue") in ROUTE_QUEUES,
        f"queue={gate.get('queue')} missing={sorted(missing) or 'none'}",
    )
    v.check(
        "R2. stuck-at-the-gate (Hinglish) -> scan, urgent",
        gate["queue"] == "scan" and gate["urgent"],
        f"queue={gate['queue']} conf={gate['confidence']} "
        f"urgent_p={gate['urgent_probability']} {gate.get('latency_ms')} ms",
    )

    refund, _ = v.call(
        "/route",
        {"message": "Please refund the unused 4 sessions of my tennis package, I'm moving city."},
    )
    v.check(
        "R3. refund request -> refund, tagged",
        refund["queue"] == "refund" and refund["refund_requested"],
        f"queue={refund['queue']} conf={refund['confidence']} "
        f"refund_requested={refund['refund_requested']}",
    )

    calm, _ = v.call(
        "/route", {"message": "What is the difference between silver and gold membership?"}
    )
    v.check(
        "R4. plan question is not urgent",
        not calm["urgent"] and calm["queue"] in ("plan", "human_review"),
        f"queue={calm['queue']} urgent_p={calm['urgent_probability']}",
    )

    try:
        v.call("/route", {"message": ""})
        v.check("R5. empty message is rejected", False, "accepted an empty message")
    except urllib.error.HTTPError as e:
        body = json.loads(e.read() or b"{}")
        v.check(
            "R5. empty message is rejected",
            e.code == 400 and body.get("ok") is False,
...
def verify_mcp_stdio(v: Verifier) -> None:
    ...
    if not exe:
        v.check(
            "MCP stdio: laya-mcp on PATH",
            False,
            "laya-mcp not found; activate the venv that has it",
        )
        return
    ...
    try:
        ...
        resp = wait_for(
            send(
                "initialize",
                {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "laya-verify", "version": "1.0"},
                },
            ),
            timeout=60,
        )
        ...
        v.check(
            "MCP stdio: tools/list", "laya_ask" in tools and "laya_plan" in tools, f"tools={tools}"
        )
        ...
        v.check(
            "MCP stdio: tools/call laya_ask",
            inner.get("choice") == "billing",
...
def main() -> int:
    ...
    p.add_argument(
        "--route",
        action="store_true",
        help="also verify POST /route (CashCard routing; MLX sidecar only)",
    )
