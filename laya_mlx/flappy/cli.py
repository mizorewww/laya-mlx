...
def play(argv=None):
    parser = argparse.ArgumentParser(
        prog="laya-flappy",
        description=__doc__,
        epilog="Also: laya-flappy benchmark --help | laya-flappy export --help",
    )
    ...
    try:
        game = FlappyGame(args.width, args.height, args.seed, args.gap)
    ...
    warm = FlappyGame(args.width, args.height, args.seed + 10000, args.gap)
    for _ in range(6):
        ...
        with Keyboard() as keys, live if live else nullcontext():
            while not quit_requested:
                ...
                if "r" in pressed:
                    stats["round"] += 1
                    game = FlappyGame(
                        args.width, args.height, args.seed + stats["round"] - 1, args.gap
                    )
                    displayed_board, displayed_decision = game.snapshot(), {}
                if stats["paused"]:
