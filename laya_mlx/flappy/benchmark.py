...
from .ui import compose, layout_size
...
def run_episode(policy, *, seed, steps, width, height, gap, fps=None, render=True):
    game = FlappyGame(width, height, seed, gap)
    buffer = io.StringIO()
    console = Console(
        file=buffer,
        force_terminal=True,
        ...
        no_color=False,
        width=layout_size(width, height)[0],
        height=layout_size(width, height)[1],
        highlight=False,
    )
    trace = []
    ...
    for index in range(steps):
        ...
        if game.score > score:
            max_pass_gap = max(max_pass_gap, game.ticks - last_pass)
            last_pass = game.ticks
        if policy.guarded and not game.alive:
            raise AssertionError("Guarded game crashed despite the lookahead shield")
        if policy.guarded and game.ticks - last_pass > game.horizon:
            raise AssertionError("Guarded game stopped passing pipes")
        trace.append(
            {
                "tick": game.ticks,
                "bird_after": [game.bird_x, game.y],
                "velocity_after": game.velocity,
                "score": game.score,
                "level": game.level,
                ...
                "crash_risk": decision.crash_risk,
                "clear_pipe": decision.clear_pipe,
    ...
    result = {
        ...
        "interventions": interventions,
        "max_completed_pass_gap": max_pass_gap,
...
def main(argv=None):
    ...
    parser.add_argument("--output", type=Path, default=Path("benchmarks/results/flappy.json"))
    ...
    warm = FlappyGame(args.width, args.height, 9001, args.gap)
    for _ in range(20):
    ...
    report = {
        ...
        "method": {
            "stable": "Zero deaths, finite outputs and continued pipe progress; <=1% ticks exceed the computation budget at each paced test rate.",
            ...
            "safety": "Laya sees planner features. Default shield restricts execution to moves with a collision-free continuation over every known pipe. Unshielded top-1 is measured separately.",
    ...
    report["method"]["uncapped_stability"] = (
        ...
        "Long episodes test finite outputs, survival and pipe progress; no fixed frame deadline."
    )
    ...
    shared = {"width": args.width, "height": args.height, "gap": args.gap}
